...
class clientAvg(ClientBase):
                ...
                if batch_idx % 4 == 0:
                    print(
                        '| Global Round: {:>2} | Local Epoch: {:>2} | Process: {:>3.0f}% | Acc: {:>3.0f}% | Loss: {:.3f}'.format(
                            global_round + 1, local_epoch + 1, 100. * (batch_idx + 1) / len(local_trainloader),
                            100. * acc / total, loss.item()))
        
        weight = copy.deepcopy(self.model.state_dict())
        weight = self.weight_update(weight)
        flattened = self.process_grad(weight)
        weight_noise = self.local_process(flattened)
    ...
    def test(self, epoch, metrics):
        self.client_evaluate(epoch, metrics)
